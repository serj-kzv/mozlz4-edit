...
var multi = function () {
    var a = function (a, b) {
        ...
        b = function (b, c, d) {
            var e = b.options[c.target.getAttribute("multi-index")];
            if (!e.disabled) {
                e.selected = !e.selected;
                var f = d.limit;
                if (f > -1) {
                    for (var g = 0, h = 0; h < b.options.length; h++) b.options[h].selected && g++;
                    if (g === f) {
                        this.disabled_limit = !0, "function" == typeof d.limit_reached && d.limit_reached();
                        for (var h = 0; h < b.options.length; h++) {
                            var i = b.options[h];
                            i.selected || i.setAttribute("disabled", !0)
                        }
                    } else if (this.disabled_limit) {
                        for (var h = 0; h < b.options.length; h++) {
                            var i = b.options[h];
                            "false" === i.getAttribute("data-origin-disabled") && i.removeAttribute("disabled")
                        }
    ...
    return function (a, d) {
        if (d = void 0 !== d ? d : {}, d.enable_search = void 0 === d.enable_search || d.enable_search, d.search_placeholder = void 0 !== d.search_placeholder ? d.search_placeholder : "Search...", d.non_selected_header = void 0 !== d.non_selected_header ? d.non_selected_header : null, d.selected_header = void 0 !== d.selected_header ? d.selected_header : null, d.limit = void 0 !== d.limit ? parseInt(d.limit) : -1, isNaN(d.limit) && (d.limit = -1), null == a.dataset.multijs && "SELECT" == a.nodeName && a.multiple) {
            ...
            h.className = "selected-wrapper", e.addEventListener("click", function (c) {
                c.target.getAttribute("multi-index") && b(a, c, d)
            }), e.addEventListener("keypress", function (c) {
                var e = 32 === c.keyCode || 13 === c.keyCode;
                c.target.getAttribute("multi-index") && e && (c.preventDefault(), b(a, c, d))
            }), e.appendChild(g), e.appendChild(h), e.non_selected = g, e.selected = h, a.wrapper = e, a.parentNode.insertBefore(e, a.nextSibling);
            for (var i = 0; i < a.options.length; i++) {
                var j = a.options[i];
                j.setAttribute("data-origin-disabled", j.disabled)
            }
            c(a, d), a.addEventListener("change", function () {
                c(a, d)
            })
        }
    }
